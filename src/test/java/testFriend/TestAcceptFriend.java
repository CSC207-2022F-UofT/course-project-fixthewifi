...
public class TestAcceptFriend {
    ...
    /**
     * Assume friend request is sent. Assert if the key code in feature can accept the friend by asserting if friend
     * list is updated
     * Friend request from user1. Let`s see if user0 has a data change
     */
    @Test
    public void testAcceptStupid(){
        Database database = new Database("testuser.csv", "testchat.csv");
        String[] a = database.readUser(0);
        //accept friend and update data
        a[6] = "-0";
        database.updateUser(0, a);
